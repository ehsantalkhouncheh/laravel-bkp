<div>
    <div class="py-4 space-y-4">
        ...
        <div class="flex justify-between">
            <div>
                <x-input.text wire:model="filters.search" placeholder="Search permissions..." />
            </div>
                ...
                <x-slot name="body">
                    @if ($selectPage)
                        <x-table.row class="bg-cool-gray-200" wire:key="row-message">
                            <x-table.cell colspan="6">
                                @unless ($selectAll)
                                    <div>
                                        <span>You have selected <strong>{{ $permissions->count() }}</strong> permissions, do you want to select all <strong>{{ $permissions->total() }}</strong>?</span>
                    ...
                    @forelse ($permissions as $permission)
                        <x-table.row wire:loading.class.delay="opacity-50" wire:key="row-{{ $permission->id }}">
                            ...
                            <x-table.cell>
                                <span class="text-cool-gray-900 font-medium">{{ $permission->guard_name }} </span>
                            </x-table.cell>

                            <x-table.cell>
                                <x-button.link wire:click="edit({{ $permission->id }})">Edit</x-button.link>
                            </x-table.cell>
                        </x-table.row>
                    @empty
                        <x-table.row>
        ...
        <x-modal.dialog wire:model.defer="showEditModal">
            ...
            <x-slot name="content">
                ...
                <x-input.group for="guard_name" label="Guard Name" :error="$errors->first('editing.guard_name')">
                    <x-input.text wire:model="editing.guard_name" id="name" placeholder="Guard Name" />
                </x-input.group>
            </x-slot>
